...
using AldursLab.WurmAssistant3.Core.ViewModels.LogSearching;
using JetBrains.Annotations;

namespace AldursLab.WurmAssistant3.Gui.Universal.Views.Legacy.LogSearching
{
    public partial class LogSearcherForm : Form, ILogSearcherModuleGui
    {
        readonly LogSearcherViewModel logSearcherViewModel;
        List<SingleSearchMatch> lastMatches = new List<SingleSearchMatch>();
        bool searching = false;
        FormWindowState lastVisibleWindowState;
        CancellationTokenSource cancellationTokenSource = new CancellationTokenSource();

        public LogSearcherForm()
        {
            InitializeComponent();
            lastVisibleWindowState = this.WindowState;
        }

        public LogSearcherForm([NotNull] LogSearcherViewModel logSearcherViewModel) 
            : this()
        {
            if (logSearcherViewModel == null) throw new ArgumentNullException("logSearcherViewModel");
            this.logSearcherViewModel = logSearcherViewModel;

            comboBoxPlayerName.Items.AddRange(
                logSearcherViewModel.Characters.Select(character => (object)character.Name).ToArray());
            comboBoxLogType.Items.AddRange(logSearcherViewModel.LogTypes.Select(type => (object)type.ToString()).ToArray());
            comboBoxSearchType.Items.AddRange(logSearcherViewModel.SearchTypes.Select(type => (object)type.ToString()).ToArray());
        ...
        async void PerformSearch()
        {
            try
            {
                ...
                var searchParams = new LogSearchParameters()
                {
                    LogType = GetLogType(),
                ...
                var result = await logSearcherViewModel.Search(searchParams, cancellationTokenSource.Token);
