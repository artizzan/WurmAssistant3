...
using AldursLab.WurmAssistant3.Core.Areas.LogSearcher.ViewModels;
...
namespace AldursLab.WurmAssistant3.Core.Areas.LogSearcher.Views
{
    ...
    {
        readonly ViewModels.LogSearcher viewModel;
        ...
        public LogSearcher([NotNull] ViewModels.LogSearcher viewModel) 
            : this()
        {
            ...
            comboBoxPlayerName.Items.AddRange(
                viewModel.Characters.Select(character => (object)character.Name).ToArray());
            comboBoxLogType.Items.AddRange(viewModel.LogTypes.Select(type => (object)type.ToString()).ToArray());
            comboBoxSearchType.Items.AddRange(viewModel.SearchTypes.Select(type => (object)type.ToString()).ToArray());
        ...
        async void PerformSearch()
        {
            try
            {
                ...
                var result = await viewModel.Search(searchParams, cancellationTokenSource.Token);
