...
{
    public partial class BreedingEvaluatorDefaultConfig : Form
    {
        private DefaultBreedingEvaluatorOptions OptionsRef;
        readonly ILogger logger;

        const string SKIP_PREGNANT_HORSES_TOOLTIP = "Pregnancy is hard to track accurately, the error can be up to a day, Assistant always assumes longest time. Horse needs to be smilexamined after breeding, else Assistant will asume longest possible pregnancy.";
        const string SKIP_PREGNANT_LAST_24H_TOOLTIP = "This adds 24h to last horse pregnancy";
        const string BAD_TRAIT_WEIGHT_TOOLTIP = "Bad trait value will be multiplied by this weight";
        const string DISCARD_WITH_ANY_BAD_TRAITS_TOOLTIP = "Skips a horse that has one or more bad traits. This does not include potential bad traits. The selected horse is not discarded, you can still click one with bad traits and get breeding suggestions against any non-bad traited horses.";
        const string INCLUDE_POTENTIAL_VALUE_TOOLTIP = "This will asume that horse has ALL traits that were not visible upon last inspection (based on inspector animal husbandry skill at that time). It is far from very useful and it is suggested to have skilled player inspect horse for you and then update it manually.";
        ...
        const string PREFER_UNIQUE_TRAITS_TOOLTOP = "This will adjust good trait value, if it's present only on one horse. This option is useful for improving your chances to get a horse with more good traits, at expense of getting overall worse horses";
        ...
        const string SKIP_PAIRED_HORSES_TOOLTIP = "If you have created any horse pairs, they will be completely excluded from breeding results";
        const string AGE_KEEP_COMPARING_OTHER_TOOLTIP = "Normally age excluded horses will not be compared at all. This will let you still see comparisons for the selected underage horse.";
        ...
        public BreedingEvaluatorDefaultConfig(DefaultBreedingEvaluatorOptions options, [NotNull] ILogger logger)
        {
            ...
            checkBoxSkipHorsesInOtherHerds.Checked = OptionsRef.IgnoreOtherHerds;
            checkBoxSkipPaired.Checked = OptionsRef.IgnorePairedHorses;
            ...
            checkBoxKeepComparingSelected.Checked = OptionsRef.AgeIgnoreOnlyOtherHorses;
            ...
            objectListViewColorWeights.SetObjects(OptionsRef.HorseColorValues);

            UpdateCtrlsEnabledStates();

            Tip(checkBoxSkipPregnant, SKIP_PREGNANT_HORSES_TOOLTIP);
            ...
            Tip(checkBoxSkipPaired, SKIP_PAIRED_HORSES_TOOLTIP);
        }
        ...
        private void buttonOK_Click(object sender, EventArgs e)
        {
            ...
            OptionsRef.IgnoreOtherHerds = checkBoxSkipHorsesInOtherHerds.Checked;
            OptionsRef.IgnorePairedHorses = checkBoxSkipPaired.Checked;
            ...
            OptionsRef.AgeIgnoreOnlyOtherHorses = checkBoxKeepComparingSelected.Checked;
