...
namespace AldurSoft.WurmApi.Impl.WurmLogFilesImpl
{
    public class LogFileInfoFactory
    {
        ...
        public LogFileInfo Create(FileInfo fileInfo)
        {
            ...
            {
                parsingError = true;
                logger.Log(LogLevel.Info, "Detected issues with parsing log date for file " + fileInfo.FullName, this);
            }

            if (typeFromLogFileName == LogType.Unspecified)
            {
                parsingError = true;
                logger.Log(LogLevel.Info, "Detected issues with parsing log type for file " + fileInfo.FullName, this);
            }
