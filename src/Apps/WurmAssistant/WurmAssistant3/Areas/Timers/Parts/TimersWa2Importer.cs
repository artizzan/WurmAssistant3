...
using AlignmentTimer = AldursLab.WurmAssistant3.Areas.Timers.Transients.Timers.Alignment.AlignmentTimer;
using MeditationTimer = AldursLab.WurmAssistant3.Areas.Timers.Transients.Timers.Meditation.MeditationTimer;
using PrayerTimer = AldursLab.WurmAssistant3.Areas.Timers.Transients.Timers.Prayer.PrayerTimer;
    ...
    public class TimersWa2Importer
    {
        ...
        private void ConfigureNewTimer(WurmTimer newTimer, Timer source)
        {
            ...
            if (Enum.TryParse(source.RuntimeTypeIdEnum, out typeId))
            {
                ...
                {
                    var t = (MeditationTimer) newTimer;
                    var s = (global::WurmAssistantDataTransfer.Dtos.MeditationTimer) source;
                ...
                {
                    var t = (AlignmentTimer) newTimer;
                    var s = (global::WurmAssistantDataTransfer.Dtos.AlignmentTimer) source;
                    t.IsWhiteLighter = s.IsWhiteLighter;
                    t.PlayerReligion =
                        (AlignmentTimer.WurmReligions)
                            Enum.Parse(typeof (AlignmentTimer.WurmReligions), s.WurmReligion, true);
                }
                else if (typeId == RuntimeTypeId.Prayer)
                {
                    var t = (PrayerTimer) newTimer;
                    var s = (global::WurmAssistantDataTransfer.Dtos.PrayerTimer) source;
