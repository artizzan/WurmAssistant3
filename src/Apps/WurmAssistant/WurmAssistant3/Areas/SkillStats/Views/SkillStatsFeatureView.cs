...
{
    public partial class SkillStatsFeatureView : ExtendedForm
    {
        readonly SkillStatsFeature feature;
        readonly IWurmApi wurmApi;
        readonly ILogger logger;

        public SkillStatsFeatureView(SkillStatsFeature feature, IWurmApi wurmApi, ILogger logger)
        {
        ...
        private async void generateQueryBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(serverGroup))
                {
                    ...
                    var view = new SkillGainsView(queryParams, reportItems);
                    view.ShowCenteredOnForm(this);
                }
            }
            catch (Exception exception)
        ...
        private async void totalSkillReportBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var view = new SkillLevelsView(queryParams, reportItems);
                view.ShowCenteredOnForm(this);
