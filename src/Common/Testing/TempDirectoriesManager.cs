...
namespace AldursLab.Testing
{
    class TempDirectoriesManager
    {
        ...
        internal void FreeHandle(DirectoryHandle directoryHandle, bool ignoreFileSystemError = false)
        {
            if (!directoryHandle.IsDisposed && directoryHandle.Dir.Exists)
            {
                ...
                {
                    if (ignoreFileSystemError) throw;
                }
            }
        ...
        ~TempDirectoriesManager()
        {
            foreach (var handle in directoryHandles.ToArray())
            {
                try
                {
                    FreeHandle(handle, ignoreFileSystemError:true);
                }
