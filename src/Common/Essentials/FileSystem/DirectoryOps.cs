...
namespace AldursLab.Essentials.FileSystem
{
    ...
    {
        public static void CopyRecursively(string sourceDirAbsolutePath, string targetDirAbsolutePath, bool overwrite = false)
        {
            var copier = new DirectoryCopier();
            if (!overwrite)
            {
                var targetDirInfo = new DirectoryInfo(targetDirAbsolutePath);
                if (targetDirInfo.Exists && targetDirInfo.EnumerateFileSystemInfos().Any())
                {
                    throw new InvalidOperationException("target directory is not empty and 'overwrite' parameter is false");
                }
            }
            
            copier.DirectoryCopy(sourceDirAbsolutePath, targetDirAbsolutePath, true);
        }
    }
