...
namespace AldursLab.Essentials.Eventing
{
    ...
    {
        /// <summary>
        /// Mark this exception as handled, continue executing message handlers.
        /// </summary>
        Handled,
        /// <summary>
        /// Continue executing message handlers, but rethrow this exception back to publisher at the end.
        /// </summary>
        ContinueThenRethrow,
        /// <summary>
        /// Rethrow this exception back to publisher right now (no further handlers will be executed).
        /// </summary>
        RethrowImmediatelly
    }
    ...
    public class MessageBus : IMessageBus
    {
        ...
        /// <summary>
        /// Global exception handler for all publish invocations.
        /// Return true to flag exception as handled.
        /// Handled exception will not break invocation chain and will not bubble back to publisher.
        /// </summary>
        public static Func<Exception, bool> GlobalExceptionHandler = exception => false;
            ...
            marshal(() =>
            {
                ...
                var dead = toNotify
                    .Where(handler =>
                    {
                        ...
                        try
                        {
                            isDead = !handler.Handle(messageType, message);
                        }
                        catch (TargetInvocationException exception)
                        {
                            if (GlobalExceptionHandler(exception.InnerException))
                            {
                                return true; // is not dead
                            }
                            throw;
                        }
                        return isDead;
